...
def rbie_test(m, bp2addr, addr2bp):
    ...
    """
    global report
    expect = report.get_expect_func("RB Port Change Interrupt", 9)

    m.breakpoint(bp2addr["main_loop"])
    m.run(MAIN_LOOP_TIMEOUT)
    m.clear_breakpoints()

    expect(m.get("INTCON") & 8, "RBIE must be set when main_loop is reached")
    ...
    def test(button):
        ...
        expect(m.get("INTCON") & 1, "RPIF must be set after clicking " + button_name)
        m.stepi(10_000)
